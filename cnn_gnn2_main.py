...
dataset = Dataset.Dataset_system(np.load('data\\sh_data.npy')[:62, :, :, :])
validdataset = Dataset.Dataset_system(np.load('data\\sh_data.npy')[48:69 ,:,:, :])
testdataset = Dataset.Dataset_system(np.load('data\\sh_data.npy')[55:,:,:, :])
device = torch.device('cuda')

batch_size = 8
dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
validdataloader = DataLoader(validdataset, batch_size=batch_size, shuffle=False)
log_file = f'log\\cnn_gnn2_training_log_{datetime.now().strftime("%Y%m%d%H%M%S")}.txt'
...
logging.info(f'{timestamp} -' + 'train start for model cnn_gnn2_stations')
adj_matrix1=torch.tensor(np.load('data\\graph_sh_sml.pkl',allow_pickle=True),dtype=torch.float32).to(device)
adj_matrix2=torch.tensor(np.load('data\\graph_sh_cor.pkl',allow_pickle=True),dtype=torch.float32).to(device)
...
for epoch in range(num_epochs):
    ...
    for inputs,q, labels in dataloader:
        ...
        q=q.to(device)
        optimizer.zero_grad()
        outputs = model.forward(inputs,q,adj_matrix1,adj_matrix2,adj_matrix3)
    ...
    if avg_val_loss.sum() < best_val_loss:
        ...
        best_model_name = f"model\\cnn_gnn2_best_model_{datetime.now().strftime('%Y%m%d%H%M%S')}.pth"
        torch.save(model.state_dict(), best_model_name)
        logging.info(f"Saved best model: {best_model_name}")

model_name = f"cnn_gnn2_final_model_{datetime.now().strftime('%Y%m%d%H%M%S')}.pth"
